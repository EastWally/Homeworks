...
namespace Task3Tests
{
    public class FibonacciTest
    {
        ...
        [Fact]
        public void GetCyclically_Positive()
        {
            // Arrange
            Dictionary<int, int> number = new Dictionary<int, int> { { 0, 0 }, { 1, 1 }, { 2, 1}, { 3, 2 }, { 4, 3 }, { 6, 8 }, { 10, 55 }, { 14, 377 } };
            Fibonacci fibonacci = new Fibonacci();
            ...
            foreach(var n in number)
            {
                int result = fibonacci.GetCyclically(n.Key);
                // Assert
                Assert.Equal(n.Value, result);
            }
        }
        ...
        [Fact]
        public void GetCyclically_Negative()
        {
            // Arrange
            int number = -1;
            Fibonacci fibonacci = new Fibonacci();
            // Assert
            Assert.Throws<ArithmeticException>(() => fibonacci.GetCyclically(number));
        }
    }
}
